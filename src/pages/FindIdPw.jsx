    ...
    <FindIdPwWrapper>
      ...
      {findState==="id"?
        <form onSubmit={(e)=>getId(e)}>
          ...
          <input type="text" value={userEmail} onChange={(e)=>setUserEmail(e.target.value)} placeholder="이메일"/>
          <button type='button' className="verify" onClick={verifyEmail}>인증 요청</button>
          <input type="text" value={userCode} onChange={(e)=>setUserCode(e.target.value)} placeholder="인증번호"/>
          <button>확인</button>
        </form>
      :
