  ...
  return (
    <Container>
        ...
        <div className='contents'>
          <h2>기본 정보</h2>
          <hr/>
          <div><h3>아이디</h3><span>id</span></div>
          <div><h3>비밀번호</h3>
          {editing?
          <>
            <input></input>
            <button className='modalBtn' onClick={()=>handleModalClick('password')}>비밀번호 변경</button>
          </>
          :<span>********</span>}
          </div>
          <div><h3>닉네임</h3>
        ...
        <div className='contents'>
          ...
          <>
            <input></input>
            <button className='modalBtn' onClick={()=>handleModalClick('email')}>인증</button>
          </>
          :<span>email@email.com</span>}
          </div>
          <div><h3>마케팅 수신 동의</h3><img src={check?'mypage_check.svg':'mypage_check_disabled.svg'}/><span>이메일</span></div>
        </div>
        {editing?
        <>
        <button className='edit cancle' onClick={()=>setEditing(false)}>취소</button>
        <button className='edit' onClick={()=>setEditing(false)}>저장</button>
        </>
        :
