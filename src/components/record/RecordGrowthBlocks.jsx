...
function RecordGrowthBlocks({props}) {
  ...
  const [statData, setStatData] = useState([
    {
      tag: <span><span className='blue'>0</span>/0일</span>,
      text: '참여일'
    },
    {
  ...
  useEffect(() => {
    if (props.thisMonth) setCurState("month")
    else setCurState("week");
  }, [props])
  ...
  useEffect(()=>{
    if (curState==="month"){
        setStatData([
            {
            tag: <span><span className='blue'>{monthState.thisMonth}</span>/{monthState.thisMonthTotalDate}일</span>,
            text: '참여일'
            },
            {
        ...
        setStatData([
            {
            tag: <span><span className='blue'>{weekState.thisWeekCount}</span>/7일</span>,
            text: '참여일'
            },
            {
